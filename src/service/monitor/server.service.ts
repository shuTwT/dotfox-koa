...
export class ServerService {
    ...
    async getInfo() {
        ...
        const memoryInfo = this.getMemInfo();
        const systemInfo = {
            computerName: os.hostname(),
            computerIp: this.getServerIP(),
            userDir: path.resolve(import.meta.dirname, '..', '..', '..', '..'),
            osName: os.platform(),
            osArch: os.arch(),
        };
        const diskInfos = await this.getDiskStatus();
        const jvmInfo = this.getMemInfo();
        const data = {
            cpuInfo,
            memoryInfo,
            systemInfo,
    ...
    getServerIP() {
        ...
        for (const name of Object.keys(nets)) {
            const netI = nets[name]
            if (netI) {
                for (const net of netI) {
                    // 选择外部可访问的IPv4地址
